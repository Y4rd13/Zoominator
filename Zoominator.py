...
while check == False:
  ...
  if len(name_input) == 2:
    name, last_name = name_input[0].capitalize(), name_input[1].capitalize()
...
root_csv = str(os.path.dirname(os.path.abspath(__file__))).replace(chr(92), '/') + '/'
utils().alert()
  ...
  while w == 1:
    ...
    for ID_class, class_hour in TL_registro_clases:
      ...
      if (class_hour < date_sec) and count < 100:
        ...
        loading_bar(r1=date_sec, r2=(class_hour - fiveMin_intoSecs), text=f'ID siguiente clase: {ID_class}')

        # kill & initialize
        handle_process(PROCNAME='Zoom.exe').kill()
        P_SUTIL(audio=audio, video=video).initialize(file, root, student_name, ID_class)
    
        # reiniciando
        loading_bar(r2=30)
        print(f'\n# R # E # I # N # I # C # I # A # N # D  O #')
        print(pd.read_csv(root_csv + csv_file))
        loading_bar(r2=100, s=0.1,text='Preparando siguiente clase...')
