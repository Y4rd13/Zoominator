...
class P_SUTIL:
    ...
    def initialize(self, file, root, student_name, ID_class):
      ...
      while checking == False and counted != 6:
        print('La barra de progreso de demorará más dependiendo de la velocidad de su computador.')
        loading_bar(r2=70 * 1, s=0.1, text=f'Trying to check process {file}, tries: {counted}, waiting: {(70 * counted * 0.1)} sec.')
...
class utils:
    ...
    def hours_left(self, class_sec, date_sec):
      ...
      min_left = ((class_sec - int(class_sec)) * 60) - ((date_sec - int(date_sec)) * 60)
...
class check_CSV:
    ...
    def check_csv_content(self, CSV_FILENAME):
      ...
      if validar in df:
        ...
        gui.alert(text = f'Por favor, hacer válido su {CSV_FILENAME} csv antes de continuar', button = 'Acepto')
